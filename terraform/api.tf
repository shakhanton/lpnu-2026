...
locals {
  api_gateway_name = "dev-lpnu-2026-api"
}
...
resource "aws_api_gateway_method" "courses_post" {
  ...
  # request_models = {
  #   "application/json" = replace("${local.api_gateway_name}-PostCourse", "-", "")
  # }
}
...
resource "aws_api_gateway_integration" "get_courses" {
  ...
  type                    = "AWS"
  uri                     = module.lambda_get_all_courses.lambda_function_invoke_arn
...
resource "aws_api_gateway_model" "post_course" {
  rest_api_id  = aws_api_gateway_rest_api.this.id
  name         = replace("${local.api_gateway_name}-PostCourse", "-", "")
...
resource "aws_api_gateway_integration" "get_authors" {
  ...
  integration_http_method = "POST"
  type                    = "AWS"
  uri                     = module.lambda_get_all_authors.lambda_function_invoke_arn
  request_parameters      = { "integration.request.header.X-Authorization" = "'static'" }
  request_templates = {
    "application/xml" = <<EOF
  {
